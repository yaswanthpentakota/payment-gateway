...
export const createPayment = async (req, res) => {
  ...
  try {
    ...
    payment = updateResult.rows

    res.status(201).json(payment)
  } catch (error) {
    console.error('Error creating payment:', error)
    res.status(500).json({
      error: { code: 'INTERNAL_SERVER_ERROR', description: 'Payment processing failed' }
    })
  }
}
...
export const listPayments = async (req, res) => {
  ...
  try {
    const query = 'SELECT * FROM payments WHERE merchant_id = $1 ORDER BY created_at DESC LIMIT 100'
